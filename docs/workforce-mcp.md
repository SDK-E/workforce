...
4. **Complete:** capability-filtered administrative application services expose objective creation, governed task creation and assignment, approval decisions, adaptive gap analysis plus probationary hiring proposals, XState employment transitions, room/membership configuration, unverified registry configuration, and a live-supervisor emergency stop. Mutations are company-scoped, idempotent with changed-request replay denial, and separately audited. Registry writes cannot forge health receipts, and a CEO emergency stop interrupts only its own company's attempts; the human control API retains the explicit global stop.
...
6. **Complete:** capability-filtered business tools let a CEO search and maintain its company's opportunities, leads, clients, and measurable delivery engagements while ARM receives read-only pipeline visibility. Mutations preserve parent isolation, audit origin, archive history, and idempotency with changed-payload replay denial. Official-SDK client tests cover the full conversion flow, discovery policy, replay, archive/restore, and cross-company denial.
7. Add MCP Inspector interoperability tests, deeper malicious-client/flood tests, and external authenticated HTTP operator access after the viable release.
