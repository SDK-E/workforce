...
Open **Organization → Agent Resources** to inspect the ARM's current reinforcement plans, review dates, latest classified decisions, and hiring proposals. An evidence-backed warning starts measurable coaching before replacement. Later recognition can complete the plan and restore active status. Repeated evidence after the plan can restrict execution and issue a formal corrective warning. Termination requires approval by default; a company can delegate that exact authority with `workforce-termination` under `policies.autonomy.authorities`. Infrastructure, provider, permission, and malformed-requirement failures are displayed as classifications but do not count as employee misconduct.
