...
Every role runs the same Alpine-based universal agent image. It contains both inference engines, Chromium, Node.js, Git, shell and audited retrieval utilities; larger language, document, PDF, image, and media toolchains are resolved from the audited tool registry into the attempt's private writable volume. The read-only image keeps only target-architecture musl engine distributions. The build gate inspects the universal image and Tinyproxy against the strict 500 MiB (524,288,000-byte) limit.
