...
Every role runs the same direct-Alpine universal agent image. It contains both inference engines, Chromium, Node.js, Git, Bash, Zsh, and audited retrieval utilities. The rootless `workforce-toolchain` resolver can add any combination of Python, PHP/Composer/Laravel/Symfony, Go, Rust, Pandoc/office/PDF, ImageMagick, and FFmpeg bundles beneath `/work/.tools`; requests are additive within the private writable volume and never modify the read-only root filesystem. The image keeps only target-architecture musl engine distributions. The verified universal image is 468,923,606 bytes, below the strict 500 MiB (524,288,000-byte) build limit.
