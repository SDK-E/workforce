...
Production profiles are separately built from Alpine for document, research, engineering, browser, and restricted-review work. The base keeps only the musl engine distributions for the target architecture. The build gate inspects every profile and the Tinyproxy image; the currently verified browser image is the largest at 522,788,577 bytes, below the strict 500 MiB (524,288,000-byte) limit.
