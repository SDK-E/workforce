...
export function planSandbox(job: JobRequirements): SandboxSpec {
  ...
  if (job.dataSensitivity === "restricted" || job.risk === "critical") profile = "restricted-review";
  else if (c.shell || c.sourceControl || c.packageInstall || c.buildTools.length) profile = "engineering";
  ...
  if (job.dataSensitivity === "restricted" && c.publicInternet) {
    ...
    decisions.push("Restricted data forbids public networking; the job must be split or explicitly re-authorized.");
  }
  if (job.risk === "critical" && (c.shell || c.packageInstall)) {
    rejectedCapabilities.push("shell", "packageInstall");
    decisions.push("Critical-risk work is review-only until a human approves a lower-level execution task.");
  }
  ...
  const networkApproved = c.publicInternet && job.network.allowedHosts.length > 0
    && job.dataSensitivity !== "restricted" && job.risk !== "critical";
  ...
  if (c.packageInstall && !rejectedCapabilities.includes("packageInstall")) tools.push("package-manager");
  tools.push(...c.buildTools.map((tool) => `build:${tool}`), ...c.languages.map((language) => `language:${language}`));

  decisions.push(`Selected ${profile} from explicit capability, risk, and sensitivity requirements.`);
  decisions.push(networkApproved ? `Network restricted to ${job.network.allowedHosts.join(", ")}.` : "Network disabled.");
  decisions.push("Workspace uses a private Docker volume; host repositories are copied in as declared inputs.");
