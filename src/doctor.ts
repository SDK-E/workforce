...
const images = docker.available ? await Promise.all([
...
console.log(`Execution readiness: ${docker.available && imagesReady ? "sandbox foundation ready; no tasks queued" : docker.available ? "build and verify images" : "start a compatible Docker daemon"}`);
