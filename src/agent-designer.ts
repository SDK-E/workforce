...
export function designAgentForJob(job: JobRequirements, manager = "arm"): AgentBlueprint {
  ...
  if (sandbox.rejectedCapabilities.length) throw new Error(`Job requirements need authorization or decomposition before hiring: ${sandbox.rejectedCapabilities.join(", ")}`);
  ...
  const skills = [
    ...
    "acceptance-verification", "workforce-collaboration"
  ...
  const instructions = [
    ...
    `Required outputs: ${job.outputs.filter((output) => output.required).map((output) => output.path).join(", ")}.`,
  ...
  return {
    employee: {
      id: `${sandbox.profile}-${suffix}`, name: `${title} ${suffix}`, title,
      department: sandbox.profile === "engineering" ? "engineering" : sandbox.profile === "research" ? "research" : "operations",
      manager, status: "probation", responsibilities: [job.objective],
      capabilityTags: [...new Set(skills)].sort(), hiredAt: new Date().toISOString()
    },
    jobId: job.id, sandbox,
    enginePolicy: { preferred: job.enginePreference[0]!, fallbacks: job.enginePreference.slice(1) },
    instructions, permissions: sandbox.tools, skills: [...new Set(skills)].sort(), probationCriteria,
    rationale: [
