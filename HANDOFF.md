...
- Workforce MCP uses the official SDK with stdio, immutable capability principals, capability-filtered discovery, bounded resources/read tools, relationship scoping, and cross-company denial tests. Agents can use joined-room chat, internal mail, meeting contributions, and assigned-task checkpoints, all with MCP-origin auditing. Complete attempt-token injection and the remaining scoped mutation/HTTP slices in `docs/workforce-mcp.md`.
...
- Execution readiness has a dedicated evidence-derived Platform view. Header/overview labels distinguish registered CEO/ARM identities from active Docker attempts, queue depth, and concurrency capacity; identity count never claims containers are running.
